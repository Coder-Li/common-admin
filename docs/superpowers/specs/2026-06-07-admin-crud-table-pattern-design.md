...
- `pageSize` must be clamped to an allowed range, for example 1 to 100.
...
- DTO validation for list query bounds and allowed fields.
...
- API client serializes list params correctly and sends bearer auth.
...
6. Add controller routes with Swagger metadata and role decorators.
...
9. Register navigation and route handling.
